...
respstr.playlist.forEach((play_list, id) => {
...
play_list.list.forEach((line_list, i) => {
   ...
   onlineGroup.tracks.push({
          ...
          ext:{
            ...
            url:`${appConfig.site}py/${ play_list.i}/${i+1}`||''
          }
...
$utils.toastError(jsonify('无播放资源'));
   return jsonify({
		list: [
			{
				title: '默认分组',
...
const Regex = /"url":"(.*?)"/;//
const url_id = data.match(Regex)[1].replace(/\\/g, "")
